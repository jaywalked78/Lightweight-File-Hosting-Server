...
DEFAULT_TIMEOUT = 43200  # 30 days in minutes - keeps directories loaded until next run
...
def load_directory(directory, server_url=None, timeout_minutes=None, verbose=True):
    ...
    if timeout_minutes is not None:
        ...
        # Use the default long timeout to ensure persistence
        payload["timeout_minutes"] = DEFAULT_TIMEOUT
    ...
    if verbose:
        ...
        print(f"  timeout_minutes: {payload['timeout_minutes']} (approx. {payload['timeout_minutes']/1440:.1f} days)")
...
def check_directory_loaded(directory_path, server_url=None):
    """Check if a directory is currently loaded on the server"""
    url = server_url or SERVER_URL
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            current_dir = server_info.get("current_directory")
            
            # Compare normalized paths
            directory_path = os.path.normpath(directory_path)
            current_dir = os.path.normpath(current_dir) if current_dir else None
            
            if current_dir == directory_path:
                return True
            else:
                return False
...
def main():
    ...
    parser.add_argument("--timeout", type=int, default=DEFAULT_TIMEOUT,
                        help=f"Set timeout in minutes (default: {DEFAULT_TIMEOUT} minutes = {DEFAULT_TIMEOUT/1440:.1f} days)")
    ...
    parser.add_argument("--force-reload", action="store_true",
                        help="Force reload directory even if already loaded")
    ...
    if verbose:
        ...
        print(f"Timeout:       {args.timeout} minutes (approx. {args.timeout/1440:.1f} days)")
    ...
    already_loaded = check_directory_loaded(source_dir, server_url)
    if already_loaded and not args.force_reload:
        print(f"Directory '{source_dir}' is already loaded on the server.")
        print("Skipping reload. Use --force-reload to reload anyway.")
    else:
        # Set timeout if specified
        if verbose:
            print(f"Setting timeout to {args.timeout} minutes (approx. {args.timeout/1440:.1f} days)...")
    ...
    if args.unload:
        ...
        if server_info.get("timeout_minutes"):
            print(f"They will be automatically unloaded after {server_info['timeout_minutes']} minutes (~{server_info['timeout_minutes']/1440:.1f} days).")
            if server_info.get("auto_unload_at"):
                print(f"Estimated unload time: {server_info['auto_unload_at']}")
        print("To unload manually, use the unload_directory.py script:")
        script_dir = os.path.dirname(os.path.realpath(__file__))
        unload_script = os.path.join(script_dir, "unload_directory.py")
        print(f"  python {unload_script} --server {server_url}")
        print("  or with verbose output:")
        print(f"  python {unload_script} --server {server_url} --verbose")
